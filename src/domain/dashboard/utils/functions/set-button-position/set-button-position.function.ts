...
export const setButtonPosition =
  ...
  (e: any) => {
    ...
    const { id, top, left, _objects: objects } = e.target;
    ...
    if (!objects) {
      ...
      objects.forEach((_target: any) => {
        ...
        if (foundButton) {
          foundButton.style.top = `${top + _target.top - buttonHeight / 2}px`;
          foundButton.style.left = `${left + _target.left - buttonWidth / 2}px`;
        }
      });
    }
  };
